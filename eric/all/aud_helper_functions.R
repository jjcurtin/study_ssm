  ...
  for(i in 1:n){
    for(j in 1:m){
      ...
      if(i==1){
        ...
        iter_dist <- prior_dist[[iter_label]]
        # Future work to remove this bound, was present in the manuscript runs, but didn't appear to be used (values didn't exceed bound)
        if(bound){
          d_lb <- -5
          d_ub <- 5
...
# Deprecated, from past work using the MARSS package
set_marss_par <- function(model,coefmat){
  flip <- check_flip(coefmat)
  model$par$R <- coefmat$R
  model$par$c <- coefmat$A
  
  if(flip){
    coef_A1 <- coefmat$Z[,1]
    coef_A2 <- coefmat$Z[,2]
    model$par$A[,1] <- coef_A2
    model$par$A[,2] <- coef_A1
    
    coef_b11 <- coefmat$B[1,1]
    coef_b22 <- coefmat$B[2,2]
    model$par$B[1,1] <- coef_b22
    model$par$B[2,2] <- coef_b11
    
    coef_d1 <- coefmat$U[1]
    coef_d2 <- coefmat$U[2]
    model$par$d[1] <- coef_d2
    model$par$d[2] <- coef_d1
  } else{
    model$par$A <- coefmat$Z
    model$par$B <- coefmat$B
    model$par$c <- coefmat$A
    model$par$d <- coefmat$U
  }
...
# Deprecated, no longer using the MARSS package directly
# MARSS fit function
run_marss_fit <- function(input_data,fit_lapse=FALSE){
  ...
  pi1 <- matrix(0, 2, 1)
  V1 = diag(1, 2, 2)
  marss_model<-list(B = B1, U = U1, Q = Q1, Z = Z1, A = A1, R = R1, x0 = pi1, V0 = V1, tinitx = 1)
  em_fitting_error <- FALSE
  em_fit <- try(MARSS(input_data, model = marss_model, control = list(maxit = 10000)),silent=TRUE)
  if(inherits(em_fit,"try-error")){
    print("encountered fatal error on EM fit, proceeding to BFGS with default init")
    bfgs_fit <- MARSS(input_data, model = marss_model, method = "BFGS",silent=TRUE)
    ...
    if(em_fit[['convergence']]==2 || em_fit[['convergence']]==52){
      ...
      bfgs_fit <- MARSS(input_data, model = marss_model, method = "BFGS", inits = em_fit,silent=TRUE)
    }
  }

  bfgs_fit[['error']] <- em_fitting_error
  bfgs_fit[['converged']] <- NA
  return(bfgs_fit)
}
...
# Future work to consider adding noise in observation equation portion of simulated trajectories
...
# Deprecated, export MARSS fit as dataframe
make_marss_coef_frame <- function(coefs,flip,fit_var=FALSE){
  #id <- info$subid
...
# Deprecated, used for checking for a particular case of degeneracy for MARSS fits
check_marss_flip <- function(coefmat){
  z1 <- coefmat$Z[,1]
  z2 <- coefmat$Z[,2]
  nz1 <-norm(z1,type='2')
  nz2 <-norm(z2,type='2')
