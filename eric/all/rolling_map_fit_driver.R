...
keyval <- ref_key
# Previous iterations had different handling for CHTC and non-CHTC
# if(run_CHTC){
#   keyval <- ref_key
# } else {
#   keyval <- ref_key
#   #keyval <- names(ref_fold_info)[1]
# }
...
tic("Runtime")
#for (train_width in seq(from=starting_width,to=starting_width+2,by=1)){
#for (train_width in seq(from=(Tfinal-first_ema_day)-1,to=(Tfinal-first_ema_day),by=1)){
for (train_width in training_seq){
  for(start_day in seq(from=first_ema_day_1i,to=(Tfinal-train_width+1),by=1)){
    ...
    if(type== 'MAP_mle'|| type=='MAP_marss' || type=='MLE'){
      ...
      # Deprecated, used for validation against MARSS fits
      # pull_marss_params_bool <-FALSE
      # if(pull_marss_params_bool){
      #   coefmat <- coef(fit_info$fit,type='matrix')
      #   mod <- set_marss_par(mod,coefmat)
      # }
      # # # # # # # # # # # #
      #     Fit/Predict     #
      # # # # # # # # # # # #
    ...
    # Deprecated, MARSS fits not used
    if(type=='MARSS'){
...
if(run_CHTC){
  ...
  print(out)
  write.csv(out,'local_test.csv',row.names=FALSE)
}
