...
get_window_preds <- function(modelfit,window_length){
  ...
  x90m<-kf$xtt[,data_length]
  x90v<-kf$Vtt[,,data_length]
...
#for(subject_idx in c(71)){
  ...
  for(horizon_idx in 1:length(horizon_list)){
    ...
    C1 <- matrix(list("c11","c21","c12","c22","c13","c23","c14","c24","c15","c25","c16","c26"),2,6)
    D1 <- 'unconstrained'
    Dlapse <- matrix(list(0),10,6)
    Dlapse[10,]<-list("d1","d2","d3","d4","d5","d6")
    pi1 <- matrix(0, 2, 1)
    V1 = diag(init_var, 2, 2)
    if(use_covar=='x'){
      chosen_model<-model2ni <- list(B = B1, U = U1, Q = Q1, Z = Z1, A = A1, R = R1,C=C1,c=covmat[,1:horizon], x0 = pi1, V0 = V1, tinitx = 1)
    } else if (use_covar=='y') {
      chosen_model<-model2ni <- list(B = B1, U = U1, Q = Q1, Z = Z1, A = A1, R = R1,D=D1,d=covmat[,1:horizon], x0 = pi1, V0 = V1, tinitx = 1)
    }else if (use_covar=='lapse') {
      chosen_model<-model2ni <- list(B = B1, U = U1, Q = Q1, Z = Z1, A = A1, R = R1,D=Dlapse,d=covmat[,1:horizon], x0 = pi1, V0 = V1, tinitx = 1)
    }else {
      chosen_model<-model2ni <- list(B = B1, U = U1, Q = Q1, Z = Z1, A = A1, R = R1, x0 = pi1, V0 = V1, tinitx = 1)
    }
...
write.csv(pred_out, "/Users/eric/repos/aud/model_eval/2ni/pred_nocovar_window.csv", row.names=FALSE)
write.csv(coef_out, "/Users/eric/repos/aud/model_eval/2ni/coef_nocovar_window.csv", row.names=FALSE)
